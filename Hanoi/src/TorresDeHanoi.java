...
public class TorresDeHanoi extends JFrame implements ActionListener, Runnable {

	public static void main(String[] args) {
		TorresDeHanoi toh = new TorresDeHanoi();
		toh.setVisible(true);
	}

	int n = Integer.parseInt(JOptionPane.showInputDialog("Ingresa el numero de discos"));
	int fwidth = 1000, fheight = 600;
	JButton start = new JButton("Start");
	JButton exit = new JButton("Exit");
	Rectangle[] peg = new Rectangle[3];
	Rectangle[] disk = new Rectangle[n];
	JLabel numof_moves = new JLabel("Numero de movimientos: ");
	JLabel present_move = new JLabel("Movimiento actual: ");
	...
	public TorresDeHanoi() {
		...
		present_move.setBounds(600, 50, 300, 25);
		add(start);
		add(exit);
		add(numof_moves);
		add(present_move);
	...
	public void Hanoi(int diskCount, int from, int dest, int by) {
		if (diskCount == 1) {
			...
			try {
				...
				repaint();
				numof_moves.setText("Numero de movimientos: " + (count++));
				present_move.setText("Movimiento actual: Disco " + (num + 1) + " se movio de " + (char) (from + 64) + " a "
						+ (char) (dest + 64));
