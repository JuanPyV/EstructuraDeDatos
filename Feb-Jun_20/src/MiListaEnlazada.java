...
public class MiListaEnlazada<E> implements Iterable<E> {
	...
	public Iterator<E> iterator() {
		return new Iterator<E>() {
			private NodoLE<E> pos = inicio;
			private boolean callRemove = false, primera = true;
			private int posN = 0;
			...
			public E next() {
				...
				if (primera) {
					this.primera = false;
					return this.pos.getDato();

				} else {
					this.callRemove = true;
					E tmp = pos.getNext().getDato();
					pos = pos.getNext();
					this.posN++;
					return tmp;
				}
	...
	public static void main(String[] args) {
		...
		MiListaEnlazada<Integer> numeritos = new MiListaEnlazada<>(lista);

		Iterator<Integer> it = numeritos.iterator();
		int num;
		while (it.hasNext()) {
			num = it.next();
			if (num == 7) {
